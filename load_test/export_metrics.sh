...
print_green "PROMETHUES EXPORTER \n======================"
if [ "$1" == "-h" ]; then
  echo  "

This script help you to get metrics from promethues.

Usage:
  sh export_metrics.sh [flags]

flags:
  -s    start time in IST (yyyy-mm-dd hh:mm:ss format)(required)
  -e    end time in IST (yyyy-mm-dd hh:mm:ss format)(required)
  -f    filename without path to export data (required)

  -a    namespaces needs to be considered ( instana-core instana-operator instana-units)
"
  exit 0
fi
...
END=""
FILE="temp"
DIR="./exported_metrics"
TIMEZONE="Asia/Kolkata"
ADD=""
while getopts 's:e:f:a:' OPTION; do
  case "$OPTION" in
    a)
      ADD="$OPTARG"
      ;;
    ...
    e)
      ...
      ;;
    ?)
      print_green "Use 'sh export_metrics.sh' -h for help  "
      exit 0
      ;;
  esac
done
...
# Convert IST to UTC
UTC_START_TIME=$(date -d "$START IST" -u "+%Y-%m-%d %H:%M:%S %Z")
UTC_END_TIME=$(date -d "$END IST" -u "+%Y-%m-%d %H:%M:%S %Z")
...
mkdir $DIR
python3 prometheus_exporter.py --namespace "$ADD" --url "https://prometheus-k8s-openshift-monitoring.apps.$(hostname -f | cut -d "." -f 2).cp.fyre.ibm.com" --start=$FORMATTED_UTC_START_TIME --end=$FORMATTED_UTC_END_TIME --keyReport --tknfile /tmp/token --filename $DIR/$FILE
rm -f prometheus_exporter.py
cat > $DIR/test.txt << EOF
TESTNAME: $FILE
TIMEZONE: $TIMEZONE
START_TIME: $START
END_TIME: $END
...
rm -fr $DIR
print_green "\nSTART TIME: $START"
print_green "\nSTART TIME IN UTC: $UTC_START_TIME"
print_green "\nEND TIME: $END"
print_green "\nEND TIME IN UTC: $UTC_END_TIME\n"
print_green "\nexported data to $FILE.tar"
print_green "\n============================"
